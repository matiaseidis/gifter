...
public class RecommendationDTO implements Serializable {
	...
	private List<GiftItemDTO> items = Lists.newLinkedList();

	public RecommendationDTO(GiftRecommendation<CanonicalCategory> r) {
		this.fill(r.getGift().getId(), r.getGift().getTitle(), r.getGift().getImageURL());
	}

	public RecommendationDTO(JSONObject data) throws JSONException {
		this.fill(data.getString("id"), data.getString("name"), data.getString("imageURL"));
	...
	public void setItems(List<GiftItemDTO> items) {
		this.items = items;
	}
